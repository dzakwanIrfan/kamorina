...
export class SavingsWithdrawalService {
  ...
  private async generateWithdrawalNumber(): Promise<string> {
    ...
    if (lastWithdrawal) {
      const lastSequence = parseInt(
        lastWithdrawal.withdrawalNumber.split('-')[2],
      );
  ...
  private async checkEarlyDepositPenalty(
    userId: string,
  ...
  async createWithdrawal(userId: string, dto: CreateSavingsWithdrawalDto) {
    ...
    const penaltyCalc = await this.checkEarlyDepositPenalty(
      userId,
  ...
  async getWithdrawalById(withdrawalId: string, userId?: string) {
    ...
    if (userId && withdrawal.userId !== userId) {
      throw new ForbiddenException(
        'Anda tidak memiliki akses ke penarikan ini',
      );
    }
  ...
  async confirmDisbursement(
    ...
    if (
      ...
      throw new BadRequestException(
        'Status penarikan tidak valid untuk disbursement',
      );
    }
    ...
    return {
      message:
        'Transaksi berhasil dikonfirmasi. Menunggu otorisasi dari Ketua.',
    };
  }
  ...
  async confirmAuthorization(
    ...
    if (withdrawal.currentStep !== SavingsWithdrawalStep.KETUA_AUTH) {
      throw new BadRequestException(
        'Penarikan tidak dalam tahap otorisasi ketua',
      );
    }

    if (
      withdrawal.status !== SavingsWithdrawalStatus.DISBURSEMENT_IN_PROGRESS
    ) {
      throw new BadRequestException(
        'Status penarikan tidak valid untuk otorisasi',
      );
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (savingsAccount) {
        ...
        const newSaldoSukarela = currentSaldoSukarela.sub(
        ...
        // Calculate transaction note based on penalty
        const withdrawalAmount = withdrawal.withdrawalAmount.toNumber();
        const penaltyAmount = withdrawal.penaltyAmount.toNumber();
        const netAmount = withdrawal.netAmount.toNumber();

        let transactionNote = `Penarikan Tabungan\nNo. Ref: ${withdrawal.withdrawalNumber}`;

        if (penaltyAmount > 0) {
          transactionNote += `\n\nRincian:`;
          transactionNote += `\nNominal: Rp ${withdrawalAmount.toLocaleString('id-ID')}`;
          transactionNote += `\nPinalti: Rp ${penaltyAmount.toLocaleString('id-ID')} (${withdrawal.penaltyRate}%)`;
          transactionNote += `\nDiterima: Rp ${netAmount.toLocaleString('id-ID')}`;
        } else {
          transactionNote += `\nNominal: Rp ${withdrawalAmount.toLocaleString('id-ID')}`;
        }
    ...
    return {
      message:
        'Penarikan tabungan berhasil diselesaikan dan dana telah ditransfer.',
    };
  }
  ...
  private async notifyApprovers(
    ...
    for (const recipient of recipients) {
      try {
        ...
        this.logger.error(
          `Failed to queue notification for ${recipient.email}:`,
          error,
        );
      }
    }
    ...
    this.logger.log(
      `Queued ${recipients.length} notifications for step ${step}`,
    );
  }
}
