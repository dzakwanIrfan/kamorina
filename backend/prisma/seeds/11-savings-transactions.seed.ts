 ...
 */
function getPastMonths(count: number): Array<{ month: number; year: number; name: string, date: Date }> {
    const now = new Date();
    const months: Array<{ month: number; year: number; name: string, date: Date }> = [];
 ...
 * 
 * Creates historical payroll data:
 * - PayrollPeriod for past 12 months (all processed)
 * - SavingsTransaction for each period per member
 * - Updates SavingsAccount balances based on transactions (Single Source of Truth)
 * - Updates MemberApplication paid status based on simulated payments
 */
export async function seedSavingsTransactions(ctx: SeederContext): Promise<void> {
    ...
    logInfo('SavingsTransactions', `Found ${verifiedUsers.length} verified members`);
    ...
    logSuccess('SavingsTransactions', `Ensured ${ctx.payrollPeriods.length} payroll periods exist`);
    ...
    for (let i = 0; i < ctx.payrollPeriods.length; i++) {
        ...
        for (const user of verifiedUsers) {
            ...
            const targetEntranceFee = new Prisma.Decimal(memberApp?.entranceFee || INITIAL_FEE);
            ...
            if (totalBalanceForInterest.gt(0)) {
                const monthlyInterestRate = new Prisma.Decimal(INTEREST_RATE).div(100).div(12);
            ...
            const totalTransactionAmount = iuranPendaftaran.add(iuranBulanan).add(tabunganDeposito);
            periodTotals[period.id] = periodTotals[period.id].add(totalTransactionAmount);
        }
    ...
    logSuccess('SavingsTransactions', `Created ${totalTransactions} savings transactions`);
    ...
    for (const user of verifiedUsers) {
        ...
        if (memberApp) {
            ...
            // Prevent remaining < 0
            const finalRemaining = remaining.lt(0) ? new Prisma.Decimal(0) : remaining;
    ...
    logSuccess('SavingsTransactions', `Synchronized balances for ${synchronizedCount} users`);
}
