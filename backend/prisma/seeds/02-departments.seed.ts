...
export async function seedDepartments(ctx: SeederContext): Promise<void> {
    ...
    ctx.departments.finance = allDepts.find(d => d.departmentName === 'Finance');
    ctx.departments.bda = allDepts.find(d => d.departmentName === 'BDA');
    ctx.departments.production = allDepts.find(d => d.departmentName === 'Production');
    ctx.departments.warehouse = allDepts.find(d => d.departmentName === 'Warehouse');
    ctx.departments.qc = allDepts.find(d => d.departmentName === 'QC');
    ctx.departments.engineering = allDepts.find(d => d.departmentName === 'Engineering');
