...
export async function seedLoanLimits(ctx: SeederContext): Promise<void> {
    ...
    for (const golongan of golongans) {
        const limitsForGolongan = LOAN_LIMITS_DATA[golongan.golonganName as keyof typeof LOAN_LIMITS_DATA];
        ...
        logSuccess('LoanLimits', `Created ${limitsForGolongan.length} loan limits for Golongan ${golongan.golonganName}`);
    }
