...
import { Plus, AlertCircle, Shield, Trash2, Download } from 'lucide-react';
...
export default function LevelsPage() {
  ...
  const [queryParams, setQueryParams] = useState<QueryLevelParams>(DEFAULT_QUERY_PARAMS);
  ...
  const handleBulkDelete = (selectedRows: Level[]) => {
    ...
    const deletableRows = selectedRows.filter((level) => level.levelName !== 'anggota');
  ...
  const handleSubmit = async (data: { levelName: string; description?: string }) => {
    try {
  ...
  const handleBulkDeleteConfirm = async () => {
    try {
      ...
      if (failCount > 0) {
        toast.error(`${failCount} levels gagal dihapus (mungkin masih memiliki user)`);
      }
  ...
  const getLevelBadgeVariant = (levelName: string): 'default' | 'secondary' | 'destructive' | 'outline' => {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            <span className="font-medium capitalize">{row.original.levelName}</span>
            {row.original.levelName === 'anggota' && (
              <span className="text-xs text-muted-foreground">Level Default</span>
            )}
          </div>
        </div>
      ),
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          Level <strong>"anggota"</strong> adalah level default dan tidak dapat dihapus.
          Setiap user baru otomatis mendapat level ini.
        </AlertDescription>
      </Alert>
