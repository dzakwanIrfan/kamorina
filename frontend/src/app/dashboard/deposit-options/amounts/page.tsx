...
import { Plus, AlertCircle, Trash2, Download } from 'lucide-react';
...
import { DepositAmountFormDialog } from '@/components/deposit-options/deposit-amount-form-dialog';
import { DeleteConfirmationDialog } from '@/components/delete-confirmation-dialog';

import { useAuthStore } from '@/store/auth.store';
import { depositOptionService } from '@/services/deposit-option.service';
import { handleApiError } from '@/lib/axios';
import { DepositAmountOption, QueryDepositOptionParams } from '@/types/deposit-option.types';
...
export default function DepositAmountsPage() {
  const { user } = useAuthStore();
  const [data, setData] = useState<PaginatedResponse<DepositAmountOption>>({
    data: [],
    meta: {
      page: 1,
      limit: 10,
  ...
  const [selectedItem, setSelectedItem] = useState<DepositAmountOption | null>(null);
  const [isBulkDeleteOpen, setIsBulkDeleteOpen] = useState(false);
  const [selectedForBulkDelete, setSelectedForBulkDelete] = useState<DepositAmountOption[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      const response = await depositOptionService.getAllAmounts(queryParams);
      setData(response);
    } catch (error) {
  ...
  const handleEdit = (item: DepositAmountOption) => {
    setSelectedItem(item);
    setIsFormOpen(true);
  };

  const handleDeleteClick = (item: DepositAmountOption) => {
    setSelectedItem(item);
    setIsDeleteOpen(true);
  };

  const handleBulkDelete = (selectedRows: DepositAmountOption[]) => {
    setSelectedForBulkDelete(selectedRows);
    setIsBulkDeleteOpen(true);
  };

  const handleExport = (selectedRows: DepositAmountOption[]) => {
    const dataToExport = selectedRows.length > 0 ? selectedRows : data.data;

    const csv = [
      ['Kode', 'Label', 'Jumlah', 'Status', 'Urutan', 'Tanggal Dibuat'],
    ...
    a.href = url;
    a.download = `deposit-amounts-${format(new Date(), 'yyyy-MM-dd')}.csv`;
    a.click();
  ...
  const handleSubmit = async (formData: any) => {
    try {
      ...
      if (selectedItem) {
        await depositOptionService.updateAmount(selectedItem.id, formData);
        toast.success('Opsi jumlah deposito berhasil diupdate');
      } else {
        await depositOptionService.createAmount(formData);
        toast.success('Opsi jumlah deposito berhasil ditambahkan');
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      const response = await depositOptionService.deleteAmount(selectedItem.id);
  ...
  const handleBulkDeleteConfirm = async () => {
    try {
      ...
      const promises = selectedForBulkDelete.map((item) =>
        depositOptionService.deleteAmount(item.id)
      );
  ...
  const tableConfig: DataTableConfig<DepositAmountOption> = {
    searchable: true,
  ...
  const columns: ColumnDef<DepositAmountOption>[] = [
    {
      accessorKey: 'code',
      header: 'Kode',
      cell: ({ row }) => (
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10">
            <FaRupiahSign className="h-4 w-4 text-primary" />
          </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Opsi Jumlah Deposito</h1>
          <p className="text-muted-foreground">
            Kelola pilihan jumlah deposito yang tersedia
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Daftar Opsi Jumlah Deposito</CardTitle>
          <CardDescription>
      ...
      <DeleteConfirmationDialog
        ...
        title="Hapus Opsi Jumlah Deposito"
