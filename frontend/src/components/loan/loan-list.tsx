...
export function LoanList({ defaultStatus, defaultStep }: LoanListProps) {
  ...
  const getDefaultStepFilter = () => {
    ...
    if (hasRole("divisi_simpan_pinjam"))
      return LoanApprovalStep.DIVISI_SIMPAN_PINJAM;
    return undefined;
  };
  ...
  const canApprove =
    hasRole("ketua") || hasRole("divisi_simpan_pinjam") || hasRole("pengawas");
  ...
  const handleBulkAction = async (
    ...
    try {
      ...
      if (result.results.failed.length > 0) {
        toast.warning(
          `${result.results.failed.length} pinjaman gagal diproses`
        );
      }
    ...
    try {
      const exportData = data.map((loan) => ({
        ...
        "Tanggal Submit": loan.submittedAt
          ? format(new Date(loan.submittedAt), "dd/MM/yyyy HH:mm", {
              locale: id,
            })
          : "-",
    ...
    filterFields: [
      {
        ...
        options: [
          ...
          {
            label: "Kredit Barang (Reimburse)",
            value: LoanType.GOODS_REIMBURSE,
          },
      ...
      },
      {
        ...
        options: [
          ...
          {
            label: "Proses Pencairan",
            value: LoanStatus.DISBURSEMENT_IN_PROGRESS,
          },
          {
            label: "Menunggu Otorisasi",
      ...
      },
      {
        ...
        options: [
          { label: "Semua Step", value: "all" },
          {
            ...
            value: LoanApprovalStep.DIVISI_SIMPAN_PINJAM,
          },
